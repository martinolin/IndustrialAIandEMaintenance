...
myGPSVibrationAccuracyDataFrame=pd.DataFrame(columns=['Assumed GPS freq/s:', 'SVM accuracy:', 'Random Forst accuracy:', 'Deep nerual network:', 'segment_duration'])
...
for segment_duration in segment_durations:
    for GPSupdateFreq in GPSUpdateFreqs:
        ...
        if GPSupdateFreq==-1:
            myGPSVibrationAccuracyDataFrame.loc[len(myGPSVibrationAccuracyDataFrame)]={'Assumed GPS freq/s:': str('GPS-distance/velocity'), 'SVM accuracy:': str(np.round(accSVM,2)), 'Random Forst accuracy:': str(np.round(accRFC,2)), 'Deep nerual network:':str(np.round(accDNN.item(),2)), 'segment_duration': str(segment_duration)}
        else:
            myGPSVibrationAccuracyDataFrame.loc[len(myGPSVibrationAccuracyDataFrame)]={'Assumed GPS freq/s:': str(np.round(GPSupdateFreq,2)), 'SVM accuracy:': str(np.round(accSVM,2)), 'Random Forst accuracy:': str(np.round(accRFC,2)), 'Deep nerual network:':str(np.round(accDNN.item(),2)),'segment_duration': str(segment_duration)}



print(myGPSVibrationAccuracyDataFrame)

myGPSVibrationAccuracyDataFrame
