...
def get_ground_level(pcd):
    ...
    _min=np.min(pcd[:,2])
    _max=np.max(pcd[:,2])
    hist, edges= np.histogram(pcd[:,2], np.arange(_min, _max + binsize,binsize))
    print("Counts per bin:", hist)
    print("Bin edges:", edges)
    myplt= matplotlib.pyplot
    plt.hist(pcd[:,2], bins=np.arange(_min, _max + binsize,binsize), edgecolor='black')
    plt.show()
    #myplt.figure()
    print("removes below:", edges[np.argmax(hist)+1])
    return edges[np.argmax(hist)+1]
